 ...
 */
function parseAuthCookies(req: NextApiRequest): {
  accessToken?: string
  refreshToken?: string
} {
 ...
 */
async function performSupabaseSignout(accessToken?: string): Promise<{
  success: boolean
  error?: any
}> {
  if (!accessToken) {
...
async function logoutHandler(req: NextApiRequest, res: NextApiResponse<LogoutResponse>) {
  ...
  try {
    ...
    if (error instanceof ValidationError) {
      ...
      return res.status(error.statusCode).json(error.toJSON() as LogoutResponse)
    }
