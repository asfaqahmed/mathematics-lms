...
interface PaymentNotifyResponse {
  success: boolean
  status: string
  message?: string
}
 ...
 */
async function validateCourse(courseId: string, expectedAmount: number): Promise<{
  id: string
  title: string
  price: number
}> {
 ...
 */
async function validateUser(userId: string): Promise<{ id: string; email: string }> {
...
async function handlePaymentStart(
  req: NextApiRequest,
  res: NextApiResponse<PaymentStartResponse>
) {
 ...
 */
async function grantCourseAccess(paymentRecord: PaymentRecord): Promise<void> {
  try {
...
async function handlePaymentNotify(
  req: NextApiRequest,
  res: NextApiResponse<PaymentNotifyResponse>
) {
  const startTime = Date.now()
 ...
 */
async function payhereHandler(req: NextApiRequest, res: NextApiResponse) {
