...
interface LessonsResponse {
  success: boolean
  data?: LessonData[]
  lessons?: LessonData[] // For compatibility
  message?: string
}
 ...
 */
async function validateCourseExists(courseId: string): Promise<void> {
 ...
 */
function transformLesson(lesson: any): LessonData {
  return {
    id: lesson.id,
 ...
 */
async function getLessons(req: NextApiRequest, res: NextApiResponse<LessonsResponse>) {
  ...
  try {
    ...
    const cached = getCached<LessonData[]>(cacheKey)
...
async function updateLesson(
  ...
  const updateData = {
    ...
    duration: parseInt(sanitizedData.duration as string) || 0,
    order,
 ...
 */
async function deleteLessons(lessonIds: string[], courseId: string): Promise<void> {
 ...
 */
async function bulkUpdateLessons(req: NextApiRequest, res: NextApiResponse<BulkUpdateResponse>) {
  const startTime = Date.now()
 ...
 */
async function lessonsHandler(req: NextApiRequest, res: NextApiResponse) {
